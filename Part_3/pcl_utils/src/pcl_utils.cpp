...
Eigen::Vector3d PclUtils::find_avg_color() {
    ...
    if(avg_color(0) >= 150 && avg_color(0) <= 255 && avg_color(1) <= 100 && avg_color(2) <= 100) {
		ROS_INFO("This is the red block");
	}
	else if(avg_color(0) <= 100 && avg_color(1) <= 100 && avg_color(2) >= 150 && avg_color(2) <= 255) {
		ROS_INFO("This is the blue block");
	}
	else {
		ROS_WARN("Cannot determine block color");
	}
    return avg_color;
